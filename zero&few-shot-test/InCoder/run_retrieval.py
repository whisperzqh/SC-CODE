...
def read_examples(file, train_data, bm25, prompt_num):
    data = []
    tem = []
    ...
    for i in range(0, len(data)):
        if run_type == 'c2t':
            code = data[i]['code']
            prompt = get_retrieval_prompt(train_data, bm25, code, prompt_num)
            tem.append(prompt + code)
        if run_type == 't2c':
            docstring = data[i]['docstring']
            prompt = get_retrieval_prompt(train_data, bm25, docstring, prompt_num)
            if language == 'matlab':
                prompt += '%' + docstring + '\n' + '<insert>'
            else:
                prompt += '#' + docstring + '\n' + '<insert>'
...
def initialize_bm25(train_data):
    if run_type == 'c2t':
        corpus = [json.loads(item)['code'] for item in train_data]
    if run_type == 't2c':
        corpus = [json.loads(item)['docstring'] for item in train_data]
    tokenized_corpus = [item.split() for item in corpus]
    bm25 = BM25Okapi(tokenized_corpus)
    return bm25


def get_retrieval_prompt(data, bm25, query, prompt_num):
...
if __name__ == '__main__':
    for run_type in ['c2t', 't2c']:
        for language in ['julia', 'matlab', 'r']:
            ...
            train_data = open('./data/' + language + '/train.jsonl', 'r', encoding='utf-8').readlines()
            bm25 = initialize_bm25(train_data)

            for prompt_num in [1, 2]:
                read_examples('./data/' + language + '/test.jsonl', train_data, bm25, prompt_num)
                total_results = []
                ...
                to_jsonl(total_results, './data/' + language + '/output_' + run_type + '_retrieval_' + str(prompt_num) + 'shot.jsonl')
                print('-------------------finish for ' + language + ' retrieval ' + str(prompt_num) + ' shot (' + run_type + ')-----------------------')
                all_examples = []