...
def code_summarization(language, mode, prompt_num):
    ...
    already_data = read_jsonl(language + '/codellama_output_' + mode + str(prompt_num) + '.jsonl')
    ...
    with open(language + '/codellama_output_' + mode + str(prompt_num) + '.jsonl', 'a',
...
def code_generation(language, mode, prompt_num):
    ...
    with open(language + '/codellama_output_' + mode + str(prompt_num) + '.jsonl', 'a',
        ...
        for i in range(len(already_data), len(data)):
            ...
            output = generate(prompt=prompt, temperature=0.9, max_new_tokens=512, top_p=0.95, repetition_penalty=1.0)
            for value in output:
                pass
            ...
            while True:
                ...
                try:
                    ...
                    if type(e) is text_generation.errors.ValidationError:
                        item = {}
                        fw = open(language + '/codellama_output_' + mode + str(prompt_num) + '.jsonl' 'a', encoding='utf-8')
                        json.dump(item, fw)
                        fw.write('\n')
                        fw.close()
                    time.sleep(2)
